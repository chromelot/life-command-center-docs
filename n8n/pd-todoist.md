...
| `Todoist Events` | `sRKv15LudXv7U2ts` | `POST /webhook/todoist-events` (real-time, from Todoist) | `item:completed` → marks PD done + creates auto-next activity on **any open deal** (`+3d` default) + DMs completer a Teams Adaptive Card (`pd-follow-up-card`) to capture outcome and refine the follow-up subject/due date. `item:uncompleted` → reopen PD. `item:updated` → if the Todoist due date differs from PD's, push it to PD (`pdPut /activities/{id}`). Echo events from PD-driven Todoist updates no-op naturally because dates already match. Cleared due dates are ignored (PD's date stays). `note:added`/`note:updated` → posts comment as a PD Note on the linked deal. |
